...
import "lib/ui/list/ObjectList";
import "lib/ui/pe/peObjectList";
import List = require("lib/ui/list/List");
import Menu = require("lib/ui/menu/Menu");
import PropertyEditor = require("lib/ui/pe/PropertyEditor");
import { INavigationService } from "lib/.core";
import EditorFactory = Slick.EditorFactory;
import EditorOptions = Slick.Editors.EditorOptions;
...
import DataRow = SlickGridDataProvider.DataRow;
import ListCommonMixin = require("lib/ui/list/ListCommonMixin");
import { ObjectListColumn } from "lib/ui/list/.list";

class SlickCellEditor {
	options: EditorOptions<DataRow>;
	parentPresenter: SlickObjectListDataPresenter;
	grid: Slick.Grid<DataRow>;
	popup: Popup;
	pe: PropertyEditor;
	protected _navigated: boolean;
	protected _handleKeys: boolean;

	/**
	 * @constructs SlickCellEditor
	 * @param options
	 */
	constructor (options: EditorOptions<SlickGridDataProvider.DataRow>) {
		var that = this,
			obj = options.item.item,
			col = (<GridColumn>options.column).source,
			$grid = $(options.grid.getContainerNode()),
		...
		that.parentPresenter = (<any>that.grid).parentPresenter;

		// create PE
		let propMeta = that._getColumnPropMeta(obj, col);
		that.pe = core.ui.PropertyEditor.DefaultMapping.create(propMeta, obj);

		// set navigation service
		let navigationService = that.parentPresenter && that.parentPresenter.navigationService;
		if (navigationService) {
			...
			navigationService = core.lang.override(Object.create(navigationService), {
				navigate: function (base, options: INavigationService.NavigateOptions) {
					that._onNavigating(options);
					return base.call(this, options);
				},
				openModal: function (base, options: INavigationService.NavigateOptions) {
					that._onNavigating(options);
					return base.call(this, options);
				}
			});
		...
		that.popup.bind("unload", () => {
			that.options.commitChanges();
		});

		that.popup.render($grid);

		let $popup = that.popup.$domElement;
		$popup.children()
	...
	destroy (): void {
		...
		if (that.pe) {
			...
			(<any>that.parentPresenter.viewModel).runItemsValidation(that.pe.viewModel);
...
//			};
	...
	_onNavigating (options: INavigationService.NavigateOptions): void {
		let that = this,
			onReturn;
		if (!that._navigated && that.popup) {
			...
			options.onReturn = function() {
				let result = onReturn.apply(this, arguments), // use this (navigationService), not that (SlickCellEditor)
	...
	_activate (): void {
		let that = this,
			$popup = that.popup && that.popup.$domElement;
		if ($popup) {
			// focus
			that.pe.focus();
			// scroll to PE
...
core.lang.override(SlickObjectListDataPresenter.prototype, {
	tweakOptions: function (base, options) {
		...
		if (options.inlineEdit) {
			options = that.mixOptions(options, (<any>SlickObjectListDataPresenter.prototype).defaultInlineOptions);

			if (!options.gridOptions.editorFactory) {
				options.gridOptions.editorFactory = {
					getEditor: function (col) {
						if (col.role !== "data" || !col.prop) { return null; }
						// if source 'editor' option is falsy, we shouldn't edit a cell
						if (col.source.editor !== undefined && !col.source.editor) { return null; }
	...
	onGridInitializing: function (base, args) {
		let that = this,
			$grid,
			pendingRows; // rows that have been changed	while an editor was opened
		...
		that.dataProvider.bind("rows.change", function (sender, args) {
			if (!args.rows || !that.grid) { return; }
			let editor = that.grid.getCellEditor();
			if (!editor) { return; }
		...
		that.grid.onBeforeEditCell.subscribe(function (e, args) {
			...
			if (!that.domElement) { return false; }

			let propMeta = that.getColumnPropMeta(args.item.item, args.column.source);
		...
		that.grid.onKeyDown.subscribe(function (e, args) {
			if (e.which === core.html.keyCode.F2) { // F2 should open inline editor
...
core.lang.extend(SlickObjectListDataPresenter.prototype, {
	 ...
	 * @returns {*}
	 */
	getColumnPropMeta: function (obj, column: ObjectListColumn) {
...
const commonListOverrides = {
	tweakOptions: function (base, options) {
		if (options.inlineEdit && !options.readOnly) {
			...
			core.lang.forEach(options.columns, (col: ObjectListColumn | string) => {
	...
	createRowMenuDefaults: function(base) {
		let that = this,
			menu: Menu.Options = base.call(that);
				...
				update: [
					{
						...
						items: [
							...
							core.lang.find(menu.items, (item: Menu.Item) => { return item.name === "Create"; })
						]
					}
				],
				remove: [ "Create" ]
...
core.lang.extend(core.ui.peObjectList.prototype, {
	doAdd: function (args) {
		var that = this,
			type = args.type || that.valueObjectEntityType.name,
