...
function initLazyImage (this: View, target: string, options: any, root: JQuery): void {
	let $element = View.findElement(target, root);
	// at the moment view's DOM can be hidden and lazyload won't show images
	let disp = this.subscribe("render", () => {
		disp.dispose();
		$element.lazyload(options);
	});
}

core.createModule("files", function (app: core.IApplication, options) {
	options = options || {};
	let baseUrl = app.dataFacade._interop.normalizeUrl(options.apiRoute);
	...
	core.files = app.files = {
		 ...
		 * @returns {String}
		 */
		getBinaryPropLoadUrl: function (obj: IDomainObject, propName: string, params?: {width?: number, height?: number}): string {
			let url,
				value = obj[propName]();
		...
		getResourceDeleteUrl: function(resourceId: string): string {
		 ...
		 */
		handleUploadError: function (jqXhr: JQueryXHR, textStatus: string, errorThrown?: string): interop.InteropError|any {
			let error = app.dataFacade._interop.handleError(jqXhr, textStatus);
			let isAbort = textStatus === "abort",
...
View.Handlebars.registerHelper("propImage", function (propName, options) {
	var viewModel = this,
		width = options.hash ? options.hash.width : undefined,
		height = options.hash ? options.hash.height : undefined,
		cssClass = options.hash ? options.hash.cssClass : undefined,
		url = "",
		attr = "",
		style = "",
		elementId,
...
View.Handlebars.registerHelper("propImageUrl", function (propName, options) {
	var viewModel = this,
		width = options && options.hash ? options.hash.width : undefined,
		height = options && options.hash ? options.hash.height : undefined;
