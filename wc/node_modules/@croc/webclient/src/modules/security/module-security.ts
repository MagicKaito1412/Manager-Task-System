...
import LoginDialog = require("./ui/LoginDialog");
import DataFacadeBase = require("lib/interop/DataFacadeBase");
import templateLoggedOut = require("xhtmpl!./ui/templates/LoggedOut.hbs");
import resources = require("i18n!lib/nls/resources");
import moduleResources = require("i18n!./nls/resources");

import "./ui/AuthMenu";
import {SecurityConfig} from "./interop";
...
core.createModule("security", function (app: core.Application): void {
	if (!app.dataFacade || !app.model) { return; }
	let dataFacade = app.dataFacade;
	let backendInterop = dataFacade._interop;

	let configSecurity: SecurityConfig = app.config["security"];
	...
	lang.extend(DataFacadeBase.prototype, /** @lends DataFacadeBase */ {
		 ...
		 */
		login: function (): lang.Promise<any> {
			let defer = lang.Deferred();
			let task = defer.then((userDto) => {
				if (!userDto) {
		 ...
		 */
		_login: function (defer: lang.Deferred<any>): void {
			let that = this;
			if (!configSecurity) {
				if (defer) {
					defer.resolve();
			...
			if (that.serverOnline && (!that.serverOnline() || that.manuallyDisconnected()) ) {
				let dto = that._currentUser || core.localStorage.getObject("security/currentUser");
				if (dto) {
			...
			that._setupLoginWaitCallbacks(defer);
			that._waitLogin.always(() => {
				that._waitLogin = undefined;
			});
		},

		createLoginDialog: function (): LoginDialog {
		...
		_openLoginDialog: function (): lang.Promise<any> {
			let that = this;
			return that.createLoginDialog().render().then((result) => {
				if (result.success) {
		...
		_setupLoginWaitCallbacks: function (defer: lang.Deferred<any>): void {
			let that = this;
			if (defer) {
				that._waitLogin
					.done((result) => {
		...
		_onLoginImpl: function (userDto: any): any {
		...
		_doCompleteLogin: function (taskOrDto, deferToSignal: lang.Deferred<any>): void {
			if (core.lang.isPromise(taskOrDto)) {
				taskOrDto.then((result) => {
		 ...
		 * @returns {Promise}
		 */
		logout: function (): lang.Promise<any> {
			if (configSecurity) {
				return this.ajax({
					...
					this.traceSource.error("DataFacade.logout error: " + error);
				});
			}
			return lang.resolved();
		},

		_doLogout: function (relogin: boolean): void {
		 ...
		 */
		getCurrentUser: function (options?: {refreshStatus?: boolean}): lang.Promise<any> {
			let that = this;
			if (that._currentUser) {
			...
			that.ajax({url: "api/_security/currentUser"}, ajaxOptions).then((response) => {
				...
				if (response) {
					...
					core.lang.async.then(that._onLogin(userDto),
						(userDto) => {
							let defer = that._waitGetCurrentUser;
				...
				let userDto = core.localStorage.getObject("security/currentUser");
				if (userDto) {
					core.lang.async.then(
						that._onLogin(userDto),
						(userDto) => {
							let defer = that._waitGetCurrentUser;
							that._waitGetCurrentUser = undefined;
							defer.resolve(userDto);
	...
	dataFacade._interop.bind("authentication_required", function (jqXhr, defer) {
		(<any>dataFacade)._login(defer);
	});

	function materializeUser(uow, userDto: DomainObjectData) {
	...
	if (app.model.UnitOfWork) {
		...
		app.model.UnitOfWork.prototype.getCurrentUser = function () {
			...
			let user = jsonOrDefer ? materializeUser(that, jsonOrDefer) : null;
			return lang.resolved(user);
		};
		app.model.UnitOfWork.prototype.getCurrentUserId = function () {
			let jsonOrDefer = <any>app.dataFacade.getCurrentUser();
	...
	function executeLogin(app: core.Application): lang.Promise<any> {
		let uow = app.createUnitOfWork();
		return uow.getCurrentUser().then(
			null,
			(err) => {
	...
	if (configSecurity && configSecurity.loginOnStart) {
		core.lang.override(app, {
			initialize: function (base, ...args) {
				let that = this;
				return core.lang.async.then(executeLogin(app), () => {
	...
	if (configSecurity && configSecurity.onLogout === "suspend") {
		app.eventPublisher.subscribe("security.logout", () => {
			// При логауте активируем специальную арию "security"
			app.areaManager.activateArea("security");
		});

		app.eventPublisher.subscribe("security.login", () => {
			// При логине уходим из арии "security"
			let areaCurrent = app.areaManager.getActiveArea();
...
function initializeArea (app: core.Application, area: Area) {
	area.transient = true;
	area.hidden(true);
...
core.createAreaModule("security", function (app: core.Application, area: Area) {
